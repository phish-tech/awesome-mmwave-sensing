...
---
## 🎨 一图胜千言：[可视化画廊](GALLERY.md)

好奇毫米波雷达底层的真实信号长什么样？点击进入我们的 **[可视化画廊](GALLERY.md)**，体验高质量、可开箱即用的硬核视觉演示：
- 🫀 **生理体征 (Vital Signs)：** 利用 EEMD 算法，从剧烈的呼吸波浪中精准剥离出极其微弱的心跳分量。
- ✋ **HCI 与手势交互 (Gestures)：** 直观呈现不同手部运动学轨迹专属的微多普勒 (Micro-Doppler) 能量特征。
- 📍 **空间追踪 (Tracking)：** 演示基于稀疏点云 (Sparse Point Cloud) 与 DBSCAN 聚类的 3D 实时多目标锁定。
...
想看看毫米波的实际效果？  
查看我们的 **[可视化画廊 (Visual Gallery)](GALLERY.md)**：
